...
get_year_bounds <- function(work.categories.in,
                            countries.in,
                            dt.in = rocks){
    print("working")
    years <- dt.in %>%
        filter(work_category %in% work.categories.in) %>%
        filter(country %in% countries.in) %>%
        pull(year)  %>%
        unique() %>%
        as.numeric()

    years.out <- list(min = min(years, na.rm = TRUE),
