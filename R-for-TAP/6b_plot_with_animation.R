...
## This adds animation in a slightly more complicated way
## -- it will take more effort to become familiar with the
##    'magick' package
## -- but I obviously think it is worth it in the long run! =)
...
additional.labels.df <- data.frame(plot.group = plot.order,
                                   country = rep(c("LAB","LAB2"),
                                                 each =3),
                                   time.hours = NA,
                                   ccode = NA,
                                   is.landlocked = NA,
                                   year = rep(unique(export.df.in$year),
                                              each = 6))
...
### Define function to add the e4t logo ------------------------------
...
make.plot <- function(df.in, year.in){

    closest_state <- as.character(year.in[[1]])
    axis.title.for.plot <- glue::glue(axis.title)

    plot.base <- df.in %>%
        ggplot() +
        geom_col(aes(x = country,
                     y = time.hours,
                     ## fill tells ggplot which variable in the
                     ## dataset to assign to the colours
    ...
    plot.name <- add.e4t.branding(plot.base,
                                 paste0(closest_state))
    return(plot.name)
}

png.list <- export.df.to.plot %>%
    group_by(year) %>%
    group_map(.f = ~make.plot(.x, .y)) %>%
    unlist()

images.df <- image_read(png.list)

animation <- image_animate(images.df, fps = 2)

image_write(animation, "E4T_export_compliance_animation_B.gif")
